...
class FaceDetection:
	...
	ConfidenceThresholdFaceDetection = 0.2  # 0.5
	...
	def detect_faces(image,display_images=False):
		# Make display_image to True to manually debug if you run into errors
		...
		for i in range(detections.shape[2]):
			...
			#                            ((ConfidenceThresholdFaceDetection = 0.5))
			#    If the model is unable to detect a valid face, reduce the threshold value
			#    If the model is detecting other objects as face or detects overlapped faces, increase the threshold value
			if confidence > FaceDetection.ConfidenceThresholdFaceDetection:
		...
		for file in os.listdir(image_dir):
			...
			try:
				...
				if len(faces) == 1:
					...
					embeddings[file.split(".")[0]] = FaceDetection.v.img_to_encoding(cv2.resize(image, (160, 160)),
					                                                                 FaceDetection.image_size)
				else:
	...
	def fetch_detections(image, embeddings, display_image_with_detections=False):
		...
		for face in faces:
			...
			img = cv2.resize(im_face, (600, 400))
		...
		if display_image_with_detections:
			cv2.imshow("Detected", cv2.resize(image, (600, 400)))
...
def face_recognition(image_or_video_path=None, display_image=False, face_dir="faces/"):
	...
	if image_or_video_path:
		...
		image_flip = True
		# cap = cv2.VideoCapture('rtsp://admin:@192.168.1.11')
...
if __name__ == '__main__':
	...
	face_recognition(args.display_image, args.faces_dir)
